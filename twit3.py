...
angs = {'ar': 'Arabic', 'bg': 'Bulgarian', 'ca': 'Catalan', 'cs': 'Czech', 'da': 'Danish', 'de': 'German', 'el': 'Greek', 'en': 'English', 'es': 'Spanish', 'et': 'Estonian',
...
class twitter_listener(StreamListener):
    ...
    def on_data(self, data):
        ...
        #     print(j["text"])
            self.counter += 1
            retweet_count = json_data["retweet_status"]["retweet_count"]
            ...
            if retweet_count >= self.retweet_count:
                ...
                self.status.add_top_tweets(self.get_tweet_html(json_data['id']))
                self.status.add_top_lang(langs[json_data["lang"]])

            if self.counter >= self.num_tweets_to_grab:
                # print(self.languages)
                # print(self.top_languages)
                # print(Counter(self.languages))
                # print(Counter(self.top_langages))
                return False
...
class TwitterMain():
    ...
    def get_streaming_data(self):
        twitter_stream = Stream(self.auth, twitter_listener(num_tweets_to_grab=self.num_tweets_to_grab, retweet_count = self.retweent_count,stats = self.stats, get_tweet_html = self.get_tweet_html ))
        try:
